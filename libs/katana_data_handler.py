...
class DataHandler:
    def __init__(self, project_root, is_path_relative=False):
    ...
    def add_sequence_asset(self, pattern_path, node, param, logger, meta=None):
        ...
        Returns (src_pattern, target_dir) if files exist, or (None, None) if nothing matched.
        """
        src_pattern = normalize_path(str(pattern_path))
        target_dir = self._target_dir(src_pattern)
        ...
        if not matched:
            logger.log("WARNING: No files matched for sequence pattern — skipping: {}".format(search))
            return None, None
